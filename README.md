...
Переносим в него задания и обработчики которые использовались для настройки хоста БД из файла reddit_app.yml.
Определяем переменную mongo_bind_ip, используемую в шаблоне файла конфигурации.
Выносим теги "db-tag" и параметр повышения прав "become: true" на уровень сценария, и удаляем их из таска.
