...
Роли app и db использованы в плейбуках packer_db.yml и packer_app.yml. Эти плейбуки использованы для создания packer-образов c провиженерами ansible. Т.к. создание packer-образов запускалось из каталога packer, при добавлении в плейбуки ролей использовались пути до них относительно каталога packer:
*playbooks/packer_db.yml:*
```  roles:
    - ../ansible/roles/db
```
*playbooks/packer_app.yml:*
```  roles:
    - ../ansible/roles/app
```
Создание образов в packer прошло удачно, на основе образов развернута stage-инфраструктура в GCP, на неё развернуто и сконфигурировано приложение, прокси и СУБД с помощью Ansible. После этого провериено подключение к внешнему адресу на http порт 80. После чего инфраструктура GCP была уничтожена.
